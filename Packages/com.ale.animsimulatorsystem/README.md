...
自 2.8.1 起，Live2D 侧的动作切换是**平滑**的。此前动作播完会在一帧内跳回 idle 姿势——根源是停止路径调用了 `CubismMotionController.StopAnimation`，它同帧断开 Playable 并清空该层的动作淡化表，而 Cubism 的淡化要求**同层至少两条动作**才介入，于是永远无法启动。

淡化分两层，各管各的：

| 场合 | 谁来淡 | 时长由谁决定 |
|---|---|---|
| **同层动作之间**（叠播新动作、播完恢复下面压着的动画） | Cubism 自己交叉淡化（本插件不再在叠播前 Stop，留住上一条当淡出源） | 各 motion 的 `.fade.asset`（`FadeInTime` / `FadeOutTime`，Cubism Editor 导出时设置；motion3.json 没写时导入默认 1 秒，短促互动建议改到 0.2~0.4 秒） |
| **跨层进出**（空层上的第一条动作淡入、动作播完回落到下层） | 本插件用**层权重**补——Cubism 的淡化管不到跨层 | `Live2DAnimator` 上的 **Live2D Layer Fade Duration**（默认 0.2 秒，与 Spine 后端「空动画淡出」一致） |

配套行为：初始化时会把 1 号及以上的空层权重清零（`CubismMotionController.OnEnable` 默认全部置 1，空层满权重会以默认姿势压住基准层）；拖拽 / 旋转 / 按压的采样通道不经层混合器，不参与上述任何淡化（见下一节第 2 条）。

排错：

- **播完仍跳变** → 包版本低于 2.8.1；或同层交叉淡化的资产链路断了——`CubismFadeController` 上没指派 fadeMotionList、`.fade.asset` 与剪辑的 `InstanceId` 事件对不上号、或模型上缺 `CubismParameterStore`。注意资产链路只影响**同层交叉淡化**，跨层回落的层权重淡出不依赖它们。
- **告警「CubismMotionController 未完成初始化」** → 缺 CubismFadeMotionList，或 Animator 上误挂了 AnimatorController（该组件要求 Animator 的 Controller 栏为空）。
- **淡化节奏不合适** → 同层交叉淡化改 `.fade.asset` 的两个时长；跨层进出改 `Live2D Layer Fade Duration`。
...
- Anim Actions：动画动作列表。配置了这个动画动作播放器中，所有的动画动作选项。每个动画动作选项，代表一个可触发的动画动作。
  ...
  - Action Range：动作的交互范围。**拖拽行程的直径**，单位是米。一般设置成2.0左右。
    注意它**不是点击范围**——点不点得到由 SphereCollider 的 Radius 决定，两者互不相干；Scene 视图里青色球是它，橙色球才是点击范围。
    - Drag(拖拽)：**只有这一种类型真正用到它**。沿动作方向从中心拖到范围边缘（即行程的一半）时，动画进度涨到100%，拖回中心则落到0%。调大它，拖满全程要走的距离就更长，动画变化也更慢。
    - Rotate(旋转)：进度由**转过的角度**决定（见下面的 Rotate Mode Angle Range Max），与光标离中心多远无关，所以这个值只需**大于0**即可，填2还是填100都一样。填0则旋转彻底失效。
    - Click(点击) / Press(按压)：**完全不使用**这个值。
    > ⚠️ **行为变更**：此前拖拽的进度换算漏除了一次半径，这个值被整个约掉——无论填多少，都固定拖满 1 米才涨到100%。默认值 2.0（半径 1 米）下两种算法恰好等价，所以这处失效一直没有暴露。修复后，此前把它改成非 2.0 的拖拽动作手感会随之改变，需要重新确认一遍。
...
自 2.8.0 起，画面**右下角有一个眼睛按钮**，点开是一列配置项，再点收起。每行从左到右是「功能名称」「显示开关」「透明度滑条」，开关默认全开、透明度默认全 1.0。

设置会**存进 `PlayerPrefs`**（六个键，前缀 `AnimSim.Display.`），下次进来还在。

面板由 `AnimSimulatorConfig` 上的 **Ui Display Config Panel Prefab** 指定，**留空则不创建**——老工程升级上来不会凭空多出一个按钮。

### 三条配置各自控什么

| 条目 | 显示开关 | 透明度 |
|---|---|---|
| **UI** | 根 `CanvasGroup` 的 Alpha 归 0 + `Blocks Raycasts` 置 false（**不停用 Canvas 物体**） | 根 `CanvasGroup` 的 Alpha，**下限 10%** |
| **操作点** | 控制提示圈**在空闲时**是否显示。关掉后空闲时缩至消失，但**鼠标悬停时放大 + 快速旋转的表现完全不变**——那是操作反馈，不是常驻提示 | 提示圈整体的透明度 |
| **操作提示** | 按下动作时是否播放手指提示。关掉后**根本不播**，也不做方向计算 | 整套提示的透明度 |

> ⚠️ **透明度是相乘的，不是绝对值。** 提示圈淡入时动画会把圈图自身的 `m_Color.a` 压到 0.78，滑条的值叠在它上面。所以滑条拉满（1.0）等于**维持现有观感**，而不是「变成全不透明」。

> **「UI」的显示开关不会停用 Canvas 物体**，只把 Alpha 归 0 并停止拦截射线。停用物体会把它下面所有组件的 `Update` 与协程一并停掉、还会走一遍 `OnDisable`/`OnEnable`，某些组件的状态会因此错乱；而 alpha 归零 + 不吃射线同样达到「看不见且点不着」。
>
> 激活态只由 `StartAnimSimulator` / `StopAnimSimulator` 负责——**两层各管一样东西**，系统级管激活、用户级管透明度与射线。

> 「UI 透明度」同时也是**淡入动画的目标值**。此前 `FadeInUI` 把 alpha 写死补间到 1，那样滑条设的值每次淡入都会被抹回不透明。

> ⚠️ **「UI」的透明度拖不到 0，下限是 10%**（`AnimSimulatorManager.UiDisplayAlphaMin`）。调到全透明会把自己锁死——根 Canvas 还激活着（显示开关仍是开的），而下面那条「点击任意处恢复」**只认显示开关、对透明度不生效**；与此同时配置面板自己也一起看不见了，只能凭记忆盲点右下角才有救。
>
> 钳制做在**管理器侧**（写入与读取都钳），面板上滑条的最小值只是让手感一致——即使有人把预制体上的`Alpha Min` 改回 0，界面也不会真的被调没。**操作点与操作提示不设下限**：它们归零时配置面板仍看得见，随时能调回来。

### 关掉 UI 之后怎么回来

**点击屏幕任意处即可恢复。** 关 UI 的用途就是把画面让出来看角色或 CG，看完随手一点就回来——所以配置面板自己也在受控 Canvas 内部，会跟着一起消失，不必留在画面上碍事。

那一次点击会被**吃掉**，不派发给角色：否则「想清屏看看画面」的那一下会顺手触发一个动作。

恢复只对**玩家自己关掉**的情况生效。`StopAnimSimulator()` 关掉的 UI 属于系统级，不会被随便一次点击唤回来——两层可见性是正交的，最终显示 = 系统级与用户级皆为真。

**配置列表展开时，点面板以外的任意位置即可收起它**，那一次点击不会派发给角色。眼睛按钮与配置列表本身算「面板之内」，点它们不触发收起——前者留给按钮自己切换，后者是正在改配置。

> **配置面板展开期间，角色交互会被挂起。** 本系统的悬停判定是**相机物理射线**、不经 GraphicRaycaster，点在 UI 上会照样穿透到后面的角色身上；拖透明度滑条尤其危险——会被识别成一次拖拽型动作。

### 接线与降级

动作列表 UI 预制体上新增了两个字段（Demo 的 `UIAnimActionList.prefab` 已接好）：

| 字段 | 指向 | 缺失时 |
|---|---|---|
| **Click Tip Idle Root** | `CircleClickTip/ImgCircleClickTip` | 关闭「操作点」时提示圈不会在空闲态消失（透明度仍生效）。真去关闭时告警一次 |
| **Operation Tip Canvas Group** | `OperationTip` 节点上的 `CanvasGroup`（新增组件） | 操作提示的透明度不生效（显示开关仍有效）。告警一次 |

沿用旧预制体**不会报错**，只是对应的那一半功能静默失效。要完整启用，把 Demo 的 `UIAnimActionList.prefab` 更新过去即可。

> **为什么是这两个节点。** `CircleClickTip` 全靠动画驱动、没有任何 C# 参与。逐条比对五个剪辑的曲线绑定后可以确认：动画写的是 `CircleClickTip` 的 `m_LocalScale` / `localEulerAnglesRaw`，以及 `ImgCircleClickTip` 的 `localEulerAnglesRaw` / `m_Color.a`——恰好**没写**前者的 CanvasGroup alpha 与后者的 scale。这两条空通道正好够用，且不会与动画抢写同一属性（抢写的结果是代码写进去当帧就被覆盖，且毫无报错）。
>
> 同理，`OperationTip` 的四个子分组的 CanvasGroup 被剪辑逐帧写死（每段剪辑都写全四组的 alpha），只有**根节点**这一层是空的，所以透明度加在根节点上。
