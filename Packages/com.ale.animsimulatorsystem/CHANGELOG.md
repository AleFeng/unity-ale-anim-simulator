...
**修复 Live2D 动作播完的姿势跳变。** 点击互动动作、待其播放完毕，角色会在一帧内跳回 idle 姿势，没有任何过渡（消费端工程报告，问题分析见其《Live2D动画混合跳变》文档）。根因：停止路径调用 `CubismMotionController.StopAnimation`，它同帧断开 Playable 并清空该层的 `_playingMotions` 淡化表——而 `CubismFadeController` 要求**同层至少两条动作**才介入混合，表被清空后淡化永远无法启动。实测修复前停止帧的参数速度是播放期常态（P90）的 **10.5 倍**（单帧尖刺），修复后降到 **0.7 倍**——低于常态波动，跳变消失。

### 修复

- **动作播完不再跳变**：轨道没有别的动画可恢复时，先把该层的**层权重淡到 0** 露出下层（时长由新增的 `Live2D Layer Fade Duration` 决定，默认 0.2 秒，与 Spine 后端 `SetEmptyAnimation(track, 0.2f)` 的空动画淡出一致），淡完再抽剪辑。Cubism 的动作淡化只发生在同层两条动作之间，跨层回落它管不了，只能用层权重自己补。
- **恢复播放走交叉淡化**：栈里压着上一条时不再「先 Stop 再 Play」——直接叠播，让 Cubism 拿残留的上一条当淡出源按 `.fade.asset` 的时长交叉淡化。实测恢复时同层动作数达到 2 并持续整个淡化窗口（此前恒 ≤1，即淡化从未发生过）。
- **起播不再硬切**：空层上的第一条动作把层权重从 0 淡入到目标值（同样是 Cubism 淡化门槛管不到的场合）；同层已有动作时权重按需补间对齐（同值零开销），交叉淡化交给 Cubism。淡出进行中重新起播会掐断在途淡出并掉头回升，权重不卡中间值。
- **清掉幻影满权重空层**：`CubismMotionController.OnEnable` 把所有层的混合器权重初始化为 1.0，空层满权重会以默认姿势压住基准层。初始化时把 1 号及以上**未被使用**的层权重清零（已播出去的初始状态不受影响）。
- **恢复分支漏带渲染器引用的隐患**：恢复播放时写回的轨道状态此前不带 `renderController`，恢复后若切入采样通道会拿不到渲染器。顺手修复。

### 说明

- **淡化职责分两层**：同层动作之间的交叉淡化归 Cubism（时长在各 motion 的 `.fade.asset` 上，Cubism Editor 导出时设置，缺省导入为 1 秒，短促互动建议 0.2~0.4 秒）；跨层进出（空层淡入 / 回落下层）归本插件的层权重补间（`Live2D Layer Fade Duration`）。README「Live2D 接入」新增「六、淡入淡出与层权重」小节详述。
- **层权重自己记账**：Cubism 没有控制器级的权重读取接口，本插件维护「层 → 当前权重」的账本，所有写入统一走记账通道。
- **误配模型的 NRE 变成一次性告警**：缺 CubismFadeMotionList 或 Animator 上误挂 AnimatorController 时，`SetLayerWeight` 会因控制器未初始化抛 NRE。新增的批量置零调用点已带防护并告警一次（提示检查模型配置）；播放路径维持既有失败面不变。
- **采样通道（拖拽 / 旋转 / 按压）不参与**：不经层混合器，维持既有的直接停止，无回归。
- 消费端若沿用把同一条 motion 同时当 idle 与动作使用的资产做法，恢复时的相位差由交叉淡化盖住；正式内容仍建议为动作配置独立 motion。

### 兼容

- **无预制体改动**。新字段 `Live2D Layer Fade Duration` 走类默认值 0.2 秒，既有预制体不需要重新保存；不想要跨层淡化的话把它设为 0 即可回到旧的立即停止（不推荐）。
- Spine / Unity 后端未触碰。
...
- **`AnimActionPlayer` 编辑期体检**（`OnValidate`）：
  - 「动画动作播放器 名称」为空——它是注册进管理器的唯一 Key，留空会与其它空名播放器互相覆盖，进度条配置也指不到它。
  - `SphereCollider` 半径不为正——**点击范围由它决定**，半径为 0 则永远点不到，而 Gizmos 画的却是另一个范围，看上去一切正常。
  - 动作的「动画名称」为空——此前要到运行期真去播时才报，且错误信息里只有一个空名字，回不到是哪一条动作配漏了。
  - 拖拽 / 旋转型动作的「动作的交互范围」不为正——交互向量长度为 0，播放进度恒为 0，拖不动也转不动，全程无任何报错。
  - 「轨道混合权重」小于 1 但播放器含拖拽 / 旋转 / 按压动作——**权重对这三种无效**（它们走逐帧采样通道、不经层混合器），此前「调小权重让动作轻一点」的意图会静默落空。
  - **同级播放器的命中球重叠**——命中判定取射线的第一个命中体，交叠区的点击归属不确定，表现为「点某处有时触发 A、有时触发 B」，是最难复现的一类问题。每对只报一次。
- **`AnimActionPlayer` 选中时额外绘制点击范围**（`OnDrawGizmosSelected`）：用橙色画出 `SphereCollider`，与既有的青色「交互行程」并列，两者都带文字标注。
  - 本组件有两个语义完全不同的「范围」：点击范围（`SphereCollider.radius`，决定点不点得到）与交互行程（动作上的 `actionRange`，只把光标位移映射成播放进度，且是**直径**）。此前 Gizmos **只画了后者**，于是「照着线框调半径却怎么都点不准」成了高频困惑。画出来比写在文档里管用。
- **`Live2DAnimator` 编辑期体检**（`OnValidate`）：
  - 「Live2D Motion Controller」未指定 / 其所在物体缺 `CubismFadeController` / `CubismFadeMotionList` 为空——这三项缺任何一个，`CubismMotionController.OnEnable` 都会提前 return 并停用自己，此后每次播放只留下一句笼统的 `can't start motion.`，那句话既不说是哪个模型也不说缺什么。
  - 同物体的 `Animator` 挂了 Animator Controller——`CubismMotionController` 检测到它就会让位，动作播放整条链路失效。而 Cubism 导入模型时**会自动生成一个空的 controller 资产**，手滑拖上去就中招。
  - 本组件与 `CubismModel` 不在同一物体——`CubismUpdateController` 只调度同物体的 `ICubismUpdatable`，分开挂会让拖拽 / 旋转 / 按压的逐帧采样被 Cubism 覆盖回起始帧。
  - 动作查找表里「剪辑」为空或动画名重复——重复的后一条会被静默忽略，按该名播放时拿到的始终是第一条。
- **进度条名查无此项时告警**（`AnimSimulatorManager.ModifyProgressBars`）：此前是静默 `return`。名字打错一个字的后果是「进度条永远不动」而动作照播，使用者只能看到「攒了半天没反应」。名字来自动作的「进度条 配置组」，是纯手填字符串，拼错是常态。按名去重，同名只报一次（进度条修改是高频调用，不去重会刷屏）。
- **条件引用的进度条不存在时告警**（两个 `AnimSim.*` 判定器）：判定器「失败即关」是刻意设计，但它意味着**名字打错与条件真的不满足表现完全一样**——动作就是不出现在列表里。报出来才能把「配错了」和「还没达标」区分开。按名去重。

### 变更

- 上述告警一律**只提示、不改写**使用者填的值——自动纠正会让人以为自己配对了。
