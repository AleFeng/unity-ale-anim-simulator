...
Ale Anim Simulator System 是一款面向 `Unity` 的 **2D 动画模拟器插件**，把美术在 **Spine / Live2D** 中制作好的动画资源、或 Unity 自带的 `AnimationClip`，变成可以用光标 **点击 / 拖拽 / 旋转 / 按压** 去玩的互动内容。
...
面向**策划**：全部配置在 Inspector 与 ScriptableObject 上完成，无需写代码；动画与皮肤一律按**字符串名**指定，**三个后端使用相同的命名规则**，因此同一份动作 / 皮肤组配置对 Spine、Live2D 与 Unity 自带动画都成立。
...
5. **三个后端一份配置** —— 动画名与皮肤名的命名规则相同，换后端只需换角色预制体上的动画控制器组件。
...
| UI 显示配置 | 画面右下角一个眼睛按钮，点开可逐项配置 **UI / 操作点 / 操作提示**的显示开关与透明度（2.8.0 起），设置存 `PlayerPrefs`。关掉 UI 后**点击屏幕任意处即可恢复**，那一次点击不会派发给角色。 |
...
| 可分组换装 | 皮肤按组配置（衣服 / 裤子 / 头发 / 眼睛 / 配饰…），每组可设最大可选数、是否必选、默认项。Spine 用骨架里的 Skin 合并成组合皮肤（**可叠加**），Live2D 用部件 ID 映射，Unity 用物体显隐组，**皮肤名规则相同**；皮肤名字段会自动列出该角色可用的下拉。 |
...
| **Unity 自带动画** | `UnityAnimator` | 无 | **引擎内置**，无需安装 | 以 Playables 自建动画图，**不需要 AnimatorController**；剪辑需**登记进查找表**；轨道自动压成层混合器的输入下标，无需手工映射；「皮肤」是物体显隐组 + 可选材质 / 贴图覆盖；进度控制与反向播放原生支持。适合 2D 精灵 / 逐帧动画、3D 模型与 UGUI。 |
...
> 三个后端的完整接入步骤（含 URP 下的额外设置）见[详细文档](#-详细文档)的「动画后端」一章。

## 💻 环境要求
- `Unity 2022.3` 或更新版本（`package.json` 声明的最低版本；本仓库基于 `Unity 6000.3` 开发与维护）。
- **至少启用一个动画后端**才能播放动画。三个都不启用时插件仍能编译，只是角色动画无法播放——编辑器加载时会给出提示。
  - 最省事的是 `ASS_UNITY_ANIM`（Unity 自带动画）：引擎内置，勾上就能用，无需安装任何运行时。
...
### 安装动画后端运行时
装完插件本体还要装动画后端的运行时，否则角色动画无法播放。三个后端**可以同时启用**，装好后在[欢迎窗口](#️-欢迎窗口)里勾上对应的宏。

**Unity 自带动画**：无需安装，引擎内置，直接勾 `ASS_UNITY_ANIM` 即可。以下两个后端才需要装运行时。
...
### 1. 启用动画后端
打开欢迎窗口（`Tools > Ale Toolkit > Anim Simulator System > Welcome`），在「动画后端」区勾上 `ASS_SPINE`、`ASS_LIVE2D` 或 `ASS_UNITY_ANIM`，等 Unity 重新编译。前两者会显示对应运行时的安装状态，未检测到时勾选会先弹确认；`ASS_UNITY_ANIM` 是引擎内置，没有安装状态一说。
...
### 3. 制作角色预制体
按 Spine / Live2D 官方流程把动画资源导入 Unity 并做成预制体（用 Unity 自带动画的话，准备好带 `Animator` 的物体与若干 `AnimationClip` 即可），然后：

- 在模型物体上挂 **`SpineAnimator`**、**`Live2DAnimator`** 或 **`UnityAnimator`**，配好它的状态数据列表（状态名 → 该状态要播的一组动画）。Live2D 与 Unity 还需额外填「查找表」。
...
- 选 `Action Operation Type`（`Click` / `Drag` / `Rotate` / `Press`），并在 `Anim Name` 里填**动画制作时起的名字**（例如 `dress-up`）——三个后端用相同的命名规则。
...
页眉之下自上而下：**「打开 Ale Toolkit 设置」跳转**、**快捷操作**（创建配置资产 / 查看使用文档 / 查看 README）、**动画后端**（`ASS_SPINE` / `ASS_LIVE2D` / `ASS_UNITY_ANIM` 一键开关，前两者实时检测对应运行时是否已安装）、**启动时自动显示**。
...
| `ASS_UNITY_ANIM` | 同上 | 无（引擎内置） | `UnityAnimator` 不参与编译，Unity 自带动画的播放与换装不可用 |

`ASS_` 是本插件自有前缀（= AnimSimulatorSystem）。**三个后端宏可以同时启用**，互不排斥。切换宏后需等待 Unity 重新编译生效。
...
>
> 但另一个方向仍需注意：**关着宏保存过的资产，其多语言条目会被丢弃**（该字段此时不参与序列化）。这一点对所有按宏门控的字段都成立，三个后端宏（`ASS_SPINE` / `ASS_LIVE2D` / `ASS_UNITY_ANIM`）同理——关着宏保存角色预制体会丢掉对应后端的配置。

> 从 2.0.0 升级上来的工程，`ASS_SPINE` 的前身叫 `HAS_SPINE`。本插件**不会**自动改写 Player Settings（那会与定义 `HAS_SPINE` 的插件互相覆盖、令编辑器反复重编译），请在欢迎窗口自行勾选 `ASS_SPINE`。

## 📖 详细文档
本 README 面向整体介绍与快速上手。**完整的使用说明**——三个后端的接入步骤、每一项配置字段的含义、UI 预制体的制作与排错、运行期自检片段等——请见插件内文档：
...
- [动画后端](Packages/com.ale.animsimulatorsystem/README.md#动画后端) — [Spine 接入](Packages/com.ale.animsimulatorsystem/README.md#spine-接入) / [Live2D 接入](Packages/com.ale.animsimulatorsystem/README.md#live2d-接入) / [Unity Animator 接入](Packages/com.ale.animsimulatorsystem/README.md#unity-animator-接入)：运行时安装、角色预制体、状态与动画、皮肤、各自的使用约束
...
- [轨道与混合权重](Packages/com.ale.animsimulatorsystem/README.md#轨道与混合权重) — 覆盖优先级的规则，以及三个后端各自的落点与边界
...
│   │                             + SpineAnimator / Live2DAnimator / UnityAnimator / AnimTrackOrdinal
